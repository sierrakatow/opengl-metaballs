...
struct VertexPNC : public VertexPN {
  ...
  VertexPNC(float x, float y, float z,
              float nx, float ny, float nz,
              float r, float g, float b)
    : VertexPN(x, y, z, nx, ny, nz), c(r, g, b) {}

  VertexPNC(const Cvec3f& pos, const Cvec3f& normal,
              const Cvec3f& color)
    : VertexPN(pos, normal), c(color) {}

  VertexPNC(const Cvec3& pos, const Cvec3& normal,
              const Cvec3& color)
    : VertexPN(pos, normal), c(color[0], color[1], color[2]) {}

  // Define copy constructor and assignment operator from GenericVertex so we can
  // use make* functions from geometrymaker.h
  VertexPNC(const GenericVertex& v) {
    *this = v;
  }

  VertexPNC& operator = (const GenericVertex& v) {
    p = v.pos;
    n = v.normal;
    c = Cvec3f(0.5,0.5,0.5);
    return *this;
  }
};
...
typedef SimpleUnindexedGeometry<VertexPNC> SimpleGeometryPNC;
...
typedef SimpleIndexedGeometry<VertexPNC, unsigned short> SimpleIndexedGeometryPNC;
